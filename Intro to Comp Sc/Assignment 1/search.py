...
def depthFirstSearch(problem):
    ...
    while problem.isGoalState(node) != True:
        node, solution, value = frontier.pop()
        ...
        if node not in exploredSet:                         
            ...
            for position, direction, value in problem.getSuccessors(node):    
                if position not in exploredSet:
                    directionlist=solution+[direction]
                    frontier.push((position, directionlist, value))
                    directionlist = list()
    ...
    while problem.isGoalState(node) != True:
        ...
        if node not in exploredSet:                         
            ...
            for position, direction, value in problem.getSuccessors(node):    
                if position not in exploredSet:
                    directionlist=solution+[direction]
                    prior = prior + problem.getCostOfActions(directionlist)
                    frontier.push((position, directionlist, value), prior)
                    directionlist = list()
                    prior = 0
    ...
    while problem.isGoalState(node) != True:
        ...
        if node not in exploredSet:                         
            ...
            for position, direction, value in problem.getSuccessors(node):  
                if position not in exploredSet:
                    directionlist=solution+[direction]
                    prior = problem.getCostOfActions(directionlist) + heuristic(position, problem)
                    frontier.push((position, directionlist, value), prior)
                    directionlist = list()
                    prior = 0
